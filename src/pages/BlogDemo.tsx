...
const Blog: React.FC = () => {
  ...
  const savePostsToStorage = (postsToSave: BlogPost[]) => {
    try {
      localStorage.setItem('blog_posts', JSON.stringify(postsToSave));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const postData: BlogPost = {
        id: editingPost || `post-${Date.now()}`,
        ...formData,
        author_id: user.id,
        author_name: user.email || 'Anonymous User',
        author_avatar: null,
        created_at: editingPost ? posts.find(p => p.id === editingPost)?.created_at || now : now,
        updated_at: now
      };

      let updatedPosts;
      if (editingPost) {
        updatedPosts = posts.map(p => p.id === editingPost ? postData : p);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50">
            ...
            {!user ? (
              <div className="bg-blue-100 text-blue-800 px-4 py-2 rounded-lg">
                <p className="text-sm font-medium">
                  <a href="/auth" className="underline hover:text-blue-600">
                    Login
                  </a> to create and share your own blog posts with the alumni community
                </p>
              </div>
            ) : user && (isAdmin || isApprovedMember) ? (
              <Button 
              ...
              <div className="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg">
                <p className="text-sm">
                  Only approved members can create blog posts. Your membership is pending approval.
                </p>
              </div>
            ) : null}
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <Card className="mb-6 bg-gradient-to-r from-blue-50 to-green-50 border-blue-200">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-4">
                Create an account to share your stories, connect with fellow alumni, and contribute to our community blog.
              </p>
        ...
        {/* Blog Post Editor */}
        {showEditor && user && (isAdmin || isApprovedMember) && (
          <Card className="mb-8">
            <CardHeader>
            ...
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-gray-500">
                  {!user 
                    ? "Login to share your own stories and experiences with fellow alumni."
                    : user && (isAdmin || isApprovedMember) 
